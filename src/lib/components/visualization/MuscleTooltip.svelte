...
{#if visible && muscle}
	<div
		class="pointer-events-none fixed z-50 max-w-xs rounded-lg border border-theme surface-elevated p-3 shadow-lg"
		style="left: {x + 12}px; top: {y + 12}px;"
	>
		<div class="mb-1 font-semibold text-primary">{muscleLabel()}</div>
		<div class="mb-2 text-sm text-secondary">{levelLabel()}</div>
			...
			<div class="text-xs text-muted">
				...
				<ul class="list-inside list-disc space-y-0.5">
					...
					{#if intensity.exercises.length > 5}
						<li class="text-muted">+{intensity.exercises.length - 5} more</li>
					{/if}
				</ul>
			</div>
		{:else}
			<div class="text-xs text-muted">No exercises target this muscle</div>
		{/if}
	</div>
{/if}
